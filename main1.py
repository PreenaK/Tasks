<html>
    <head>
        <h1 class="center">This is all about ME and MY PPL</h1><br>
        <div>
            <img src="https://media.istockphoto.com/id/700702502/photo/girlfriends-in-the-city.jpg?b=1&s=612x612&w=0&k=20&c=lXeYNGGPKCQdDYoYduVY86cRD1jbcaoIAz3_1S2ob4s="alt="FRIENDSHIP" class="center1">
        </div>
        <br>
        <div class="center1">
            <h2 class="center2">Dialy Routine Of My Friends</h2>
            <br>
            <ul class="color">
                <li class="color1">Wake up early in the morning.</li>
                <li class="color1">Phone</li>
                <li class="color1">Fresh UP</li>
                <li class="color1">Packing things</li>
                <li class="color1">Break Fast</li>
                <li class="color1">College </li>
                <li class="color1">Break</li>
                <li class="color1">Going out</li>
                <li class="color1">Back To Home</li>
                <li class="color1">Spending Time With Family</li>
                <li class="color1">dinner</li>
                <li class="color1">Complete Work Given In College</li>
                <li class="color1">Sleep</li>
            </ul>
        </div><br><br><br><br>
        <form>
            <fieldset>
                <legend class="center2">My FRIENDS Personal INFO</legend><br>
                <table class="color2" border="5" bordercolor="blue">
                    <tr>
                        <th>Name</th>
                        <th>Birthday</th>
                        <th>Sport</th>
                        <th>Food</th>
                        <th>Destination</th>
                        <th>Hero</th>
                        <th>Since</th>
                    </tr>
                    <tr>
                        <td>Myself</td>
                        <td>26-12-2003</td>
                        <td>Cricket</td>
                        <td>McDonald's </td>
                        <td>Paris</td>
                        <td><img src="https://img.mensxp.com/media/content/2022/Dec/Ranbir-Kapoor-Talks-About-His-10-Year-Plan1200_6391906d58119.jpeg" alt="Mr.Kapoor"></td>
                        <td>19 years</td>
                    </tr>
                    <tr>
                        <td>Shriya</td>
                        <td>19-08-2004</td>
                        <td>Badminton</td>
                        <td>kfc</td>
                        <td>paris</td>
                        <td><img src="https://img.mensxp.com/media/content/2022/Dec/Ranbir-Kapoor-Talks-About-His-10-Year-Plan1200_6391906d58119.jpeg" alt="Super Start"></td>
                        <td>15 years</td>
                    </tr>
                    <tr>
                        <td>Shruti</td>
                        <td>25-04-2003</td>
                        <td>cricket</td>
                        <td>kfc</td>
                        <td>Paris</td>
                        <td><img src="https://img.mensxp.com/media/content/2022/Dec/Ranbir-Kapoor-Talks-About-His-10-Year-Plan1200_6391906d58119.jpeg"></td>
                        <td>15 years</td>
                    </tr>
                </table>
            </fieldset>
        </form>
    </body>
</html>